// minml CUDA shim — single .cu file. Allocates `minml_cuda_buf` on the
// host heap (carrying a `void* device_ptr` to the cudaMalloc'd region);
// the addon calls minml_cuda_free when the JS-side External finalizer
// fires.
